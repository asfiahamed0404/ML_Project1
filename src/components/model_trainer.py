...
class ModelTrainer:
    ...
    def initiate_model_trainer(self, train_array, test_array):    #these arguments are from tranformed data
        try:
            logging.info("Splitting training and test input data")
            X_train, y_train, X_test, y_test = (
                train_array[:, :-1],# in train and test array the last is target feature and all other are input features
                train_array[:, -1],
                test_array[:, :-1],
                test_array[:, -1],
            )
